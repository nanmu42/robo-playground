...
class KeeperMind(rm.Worker):
    ...
    def _dequeue_vision(self):
        while not self.closed:
            try:
                self._ball_distances = self._vision.get(timeout=QUEUE_TIMEOUT)
            except queue.Empty:
                continue
            break
    ...
    def _dequeue_push(self):
        ...
        while not self.closed:
            try:
                push = self._push.get(timeout=QUEUE_TIMEOUT)
            except queue.Empty:
                continue
            break
    ...
    def _dequeue_event(self):
        ...
        while not self.closed:
            try:
                ...
                # event is generated by triggering
                return
...
@click.option('--max-width', default=0.6, type=float, help='(Optional) Field width')
@click.option('--max-depth', default=0.6, type=float, help='(Optional) Field depth')
@click.pass_context
